function createProgressBlock(book) {
  ...
  pages.textContent = book.page_count
    ? `Página ${Number(book.current_page) || 0} de ${book.page_count}`
    : `Página ${Number(book.current_page) || 0}`;
...
function renderLibrary() {
  ...
  elements.librarySummary.textContent = configured && !state.user
    ? `${label} en la demostración. Inicia sesión para cargar tu biblioteca real.`
    : label;
  updateStats();
}
